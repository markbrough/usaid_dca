...
(function($) {
  ...
  OpenSpending.Search = function(config) {
    ...
    my.entryUrl = function(entry) {
      ...
      return my._config.endpoint + 'usaid_dca/entries/' + entry_id;
    };
    ...
    my.entityUrl = function(entity) {
      ...
      return my._config.endpoint + 'usaid_dca/from/' + entity_id + '?q=time.year%3A' + parent.year;
    };
    ...
    my.sectorShow = function(entity) {
      var out = $('<div />').append(
          $('<a />').text(entity.label).attr('href', (my._config.endpoint + 'usaid_dca/sector/' + entity.name + '?_time=' + parent.year)).attr("target","_blank")
        ).html();
      return out
    };

   my.countryShow = function(entity) {
      var out = $('<div />').append(
          $('<a />').text(entity.label).attr('href', (my._config.endpoint + 'usaid_dca/country/' + entity.name + '?_time=' + parent.year)).attr("target","_blank")
        ).html();
      return out
    };

   my.adminShow = function(entity) {
      var out = $('<div />').append(
          $('<a />').text(entity.label).attr('href', (my._config.endpoint + 'usaid_dca/Admin1/' + entity.name + '?_time=' + parent.year)).attr("target","_blank")
        ).html();
      return out
    };
        ...
        var entries = $.map(data.response.docs, function(item, idx) {
          ...
          for (key in item) {
            ...
            if (key.indexOf('country.') === 0) {
              entry.country[key.slice(8)] = item[key];
            }
            if (key.indexOf('Admin1.') === 0) {
              entry.admin1[key.slice(7)] = item[key];
            }
          }
          return entry;
        });
        ...
        <table> \
          <thead> \
            <tr> \
              ...
              <th column="country.label" class="sortable">Country</th> \
              <th column="admin1.label">Admin1</th> \
          ...
          <tbody> \
            ...
            <tr> \
              <td> \
                {{html my.sectorShow(entry.sector)}} \
              </td> \
              <td> \
                {{html my.countryShow(entry.country)}} \
              </td> \
              <td class="{{html entry.admin1}}"> \
                {{html my.adminShow(entry.admin1)}} \
              </td> \
              <td class="num"> \
